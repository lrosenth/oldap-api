...
- Generic publication (source implementation, not globally activated): configured publisher roles plus resource UPDATE, atomic root/media publication and exact receipts. See doc/archive_publication.md. Matching library/API/all-writer releases and policy opt-in remain required; no ontology change.
...
- AS-03 adds `GET /data/{project}/staging-folder-inventory` with the frozen mixed-entry schema, revision-bound signed cursors and no-store errors. Existing transform bodies/results now invoke atomic retained-reference and organisation-read preservation in oldaplib. Mobile commit receipts replay unchanged after transfer/relocation; late notes retain 409. See `doc/staging_folder_inventory.md`. No CaptureApp source, ontology, live ACL or deployment changes; UI/native acceptance and activation remain later steps.

- AS-02 backend source is complete with the matching oldaplib checkout. Generic updates preserve legacy formats, use typed errors and enforce post-catalogue note rejection before conversion. Additive capabilities, atomic reference moves and owner-scoped receipt routes use the frozen v1 schema. Selecting `OLDAP_ARCHIVE_POLICY_FILE` enables shared persistent writer coordination for API/direct-library workers; durable Redis deployment and controlled recovery are mandatory. See `doc/archive_update_boundary.md` and oldaplib `docs/writer_recovery.md`. No release, policy activation, ACL migration or CaptureApp change occurred; AS-03 supplies transfer references/mixed inventory.
